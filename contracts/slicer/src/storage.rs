...
use crate::types::{Session, StorageKey};
...
pub fn get_attestor(env: &Env) -> Option<BytesN<32>> {
    env.storage().instance().get(&StorageKey::Attestor)
}

pub fn set_attestor(env: &Env, pubkey: &BytesN<32>) {
    env.storage().instance().set(&StorageKey::Attestor, pubkey);
}
...
pub fn get_targets(env: &Env) -> Vec<TargetEntry> {
    env.storage().instance().get(&StorageKey::Targets)
